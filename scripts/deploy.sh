...
WASM="target/wasm32-unknown-unknown/release/chainbet.wasm"
cd ..
...
echo -e "${YELLOW}[6/6] Creating proof bet...${NC}"
TX_RESULT=$(stellar contract invoke \
  ...
  --creator ${DEPLOYER} \
  --oracle ${ORACLE} \
  --description '"Stellar will hit 10M daily active wallets before end of 2026"' \
  --creator_stake 5000000 \
  --counterparty_stake 5000000 \
